...
def State3():
    ...
    time = str(timenow)
    print(time)
    socket.send_json(["logic","Hover_Spin_Up", Value, time])
...
def State4():
    ...
    time = str(timenow)
    print(time)
    socket.send_json(["logic","Accelerate_Forward", Value, time])
...
def State5():
    ...
    time = str(timenow)
    print(time)
    #socket.send_json(["logic","Braking_1", Value, time])
    socket.send_json(["logic", "Braking_1", Value, time])
...
def State13():
    ...
    time = str(timenow)
    print(time)
    socket.send_json(["logic","Emergency_Shutdown", Value, time])
...
def State99():
    ...
    if result == True:
        ...
        time = str(timenow)
        print(time)
        socket.send_json(["logic","Emergency_Override", Value, time])
    else:
...
button99 = Tk.Button(window, text = "Emergency Override", fg = "black", bg = "gold", command = State99)
