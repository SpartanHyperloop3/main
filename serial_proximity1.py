# Filename: proxModule.py
# Credits to John Bohlhuis for the boilerplate code
...
def measure(portName):
    ...
    while time() < timeStart + maxwait:
        if ser.inWaiting():
            ...
            valueCount += 1
            if valueCount < 2: # Error filter as the 1st reading may be partial number; throw it out
                continue
            testData = ser.read(bytesToRead)
            if not testData.startswith(b'R'):
                # if data received did not start with R, ditch it
                continue
            try:
